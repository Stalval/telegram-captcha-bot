...
async def welcome_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for member in new_members:
        ...
        captcha_caption = f"Добро пожаловать, {member.mention_html()}! У вас есть {CAPTCHA_TIMEOUT_SECONDS} секунд, чтобы подтвердить, что вы не робот."
        ...
        context.job_queue.run_once(
            ...
            data={'chat_id': chat.id, 'captcha_message_id': captcha_message.message_id, 'join_message_id': join_message_id},
            name=job_name,
        )
...
async def delete_captcha_timeout(context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f"Сработал таймаут капчи в чате {job_data['chat_id']}. Удаление сообщений.")
    try:
        await context.bot.delete_message(chat_id=job_data['chat_id'], message_id=job_data['captcha_message_id'])
    except BadRequest:
        logger.warning(f"Не удалось удалить сообщение с капчей (возможно, уже удалено).")
    try:
        await context.bot.delete_message(chat_id=job_data['chat_id'], message_id=job_data['join_message_id'])
    except BadRequest:
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "correct":
        try:
            ...
            await query.delete_message()
            welcome_text = f"""✅ Привет, {user.mention_html()}! Проверка пройдена...""" # Сокращено для примера
